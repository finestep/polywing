    ...
    public void Deform(Circle c)
    {
        ...
        for (int pathI = 0; pathI < td.polyPathCount; pathI++)
        {
            ...
            do
            {
                ...
                double quadA = delta.sqrMagnitude;
                double quadB = 2 * (Vector2.Dot(pos, delta) - Vector2.Dot(c.pos, delta));
                double quadC = pos.sqrMagnitude - 2 * Vector2.Dot(pos, c.pos)
                                + c.pos.sqrMagnitude - c.rad * c.rad;
                float discr = (float)( quadB * quadB - 4 * quadA * quadC);
                ...
                if (discr > 0)
                {
                    ...
                    float alpha1 = (float)((-quadB - Mathf.Sqrt(discr)) / (2 * quadA));
                    float alpha2 = (float)((-quadB + Mathf.Sqrt(discr)) / (2 * quadA));
                    ...
                    if (Mathf.Abs(alpha1 - alpha2) > 0.01f)
                    {
